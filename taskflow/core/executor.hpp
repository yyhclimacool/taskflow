...
inline Executor::Executor(size_t N, std::shared_ptr<WorkerInterface> wix)
    : _MAX_STEALS{((N + 1) << 1)}, _threads{N}, _workers{N}, _notifier{N}, _worker_interface{std::move(wix)} {
  if (N == 0) {
...
auto Executor::named_async(const std::string& name, F&& f, ArgsT&&... args) {
  ...
  auto node = node_pool.animate(
    ...
    [p = make_moc(std::move(p)), f = std::forward<F>(f), args...](bool cancel) mutable {
...
void Executor::named_silent_async(const std::string& name, F&& f, ArgsT&&... args) {
  ...
  Node* node = node_pool.animate(std::in_place_type_t<Node::SilentAsync>{},
...
inline void Executor::_spawn(size_t N) {
  ...
  for (size_t id = 0; id < N; ++id) {
    ...
    _threads[id] = std::thread(
      [this](Worker& w, std::mutex& mutex, std::condition_variable& cond, size_t& n) -> void {
        // assign the thread
...
tf::Future<void> Executor::run_until(Taskflow& f, P&& p, C&& c) {
  ...
  if (empty || p()) {
    ...
    promise.set_value();
    _decrement_topology_and_notify();
    //! 这里wait能直接得到结果，因为promise已经set_value了
...
template <typename F, typename... ArgsT>
auto Subflow::_named_async(Worker& w, const std::string& name, F&& f, ArgsT&&... args) {
...
void Subflow::_named_silent_async(Worker& w, const std::string& name, F&& f, ArgsT&&... args) {
  ...
  auto node = node_pool.animate(std::in_place_type_t<Node::SilentAsync>{},
                                [f = std::forward<F>(f), args...]() mutable { f(args...); });
