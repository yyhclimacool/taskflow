...
// Constructor
inline Taskflow::Taskflow(const std::string& name) : FlowBuilder{_graph}, _name{name} {}
...
class Future : public std::future<T> {
  ...
  using handle_t = std::variant<std::monostate, std::weak_ptr<Topology>, std::weak_ptr<AsyncTopology>>;
